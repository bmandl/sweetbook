...
function Center(props) {
    ...
    let data = props.body;
    let steps = data.steps.map((step, index) => <ListGroup.Item key={'step' + index}>{step}</ListGroup.Item>)
    let ingredients = data.ingredients.map((ingredient, index) => <ListGroup.Item key={'ingredient' + index}>{ingredient}</ListGroup.Item>)
    return (
        <Modal {...props} size="lg" centered>
            ...
            <Modal.Body>
                <h2>Brief description</h2><p>{data.brief}</p>
                <h2>Ingredients</h2><ListGroup as="ul">{ingredients}</ListGroup>
                <h2>Preparation</h2><ListGroup as="ul">{steps}</ListGroup>
                <h2>Details</h2>
                <h3>Type:</h3><p>{data.details.type}</p>
                <h3>Main ingredients:</h3><p>{data.details.main}</p>
                <h3>Occasion:</h3><p>{data.details.occasion}</p>
                <h3>Method:</h3><p>{data.details.method}</p>
                <h3>Diet:</h3><p>{data.details.diet}</p>
            </Modal.Body>
        </Modal>
    )
}
...
const AddRecipe = () => {
    ...
    const [keyIngredient, updateKeyIng] = useState(1);
    const [keyStep, updateKeyStep] = useState(1);
    ...
    async function addItem(fields) {
        ...
        let action = { type: 'add', payload: <ListGroup.Item name={values[0]} key={keys[0] == 'ingredient' ? keyIngredient : keyStep} ref={register} onClick={removeItem}>{values.join(' ')}</ListGroup.Item> };
        switch (keys[0]) {
            case 'ingredient':
                updateIngredients(action)
                updateKeyIng(keyIngredient + 1)
                break;
            case 'step':
                updateSteps(action)
                updateKeyStep(keyStep + 1)
                break;
            default:
                break;
        }
    ...
    return (
        <><Center body={recipeData} show={modalShow} onHide={() => setModalShow(false)} />
